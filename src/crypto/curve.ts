...
function trackPoint<T extends object>(wrapper: object, inner: T): T {
  ...
  // registry entry before freeing.  Without this, the GC-driven callback
  // can fire AFTER explicit cleanup, calling __destroy__ on memory that has
  // already been reused — silently corrupting whichever object now lives at
  // that address (e.g. the static generator).

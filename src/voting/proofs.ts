...
export function proveOR(
  ...
  for (let i = 0; i < B1; i++) {
    if (i === witness.trueIndex) {
      ...
      // All intermediates are freed immediately after use so they don't
      // accumulate on the 16 MB WASM heap across many candidates.
      const sim = sims[i] ?? {
        e: randomScalar(),
        z: randomScalar(),
      };
...
export function proveBudgetExact(
  ...
  // Free the two freshly-allocated points (P2 = dleqStmt.base1, D = dleqStmt.point2).
  // base2 is stmt.mpk and point1 is stmt.ctSum.c1 — both owned by the caller.

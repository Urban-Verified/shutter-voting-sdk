...
export function buildBallot(args: BuildBallotArgs): BallotInputs {
  ...
  if (params.variant === 'A') {
    ...
    // Variant B: weighted sum Σ_k 2^k · c_{j,k} per candidate, then Σ_j ĉ_j.
    // Explicitly free all intermediate weighted-mul G2 points so they don't
    // accumulate on the fixed 16 MB WASM heap.
    const d = params.d!;
    rSum = 0n;
  ...
  // For atMost budget proof, also free its branch G2 commitment points.
  ...
  // After extracting bytes, ciphertext G2 points are no longer needed.
  for (const ct of cts) {
    ct.c1.destroyWasm();
    ct.c2.destroyWasm();
  }
  ...
  sig.R.destroyWasm(); // R is encoded to bytes; free the WASM allocation
